...
#define HM_FRM_DIMMER_INFO_LEVEL_NEW(frm_buf_tgt, frm_cnt, frm_dst_addr) \
	HM_FRM_NEW((frm_buf_tgt), 14, frm_cnt, 0x10, (frm_dst_addr)); \
	(frm_buf_tgt).dimmer_info_level.subtype = 0x06; \
	(frm_buf_tgt).flags.bcast = 1;
typedef struct {
	uint8_t subtype;
	uint8_t channel;
	uint8_t level;
...
#define HM_FRM_DIMMER_SET_IS(frm) ((frm).type == 0x11 && (frm).dimmer_set.subtype == 0x02)
#define HM_FRM_DIMMER_SET_NEW(frm_buf_tgt, frm_dst_addr) \
	HM_FRM_NEW((frm_buf_tgt), 16, ++hm_frm_cnt, 0x11, (frm_dst_addr)); \
