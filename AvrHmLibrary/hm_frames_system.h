...
#define HM_DEVICE_CLASS_SWITCH 0x10
#define HM_DEVICE_CLASS_DIMMER 0x20
#define HM_DEVICE_CLASS_BLIND_ACTUATOR 0x30
#define HM_DEVICE_CLASS_REMOTE 0x40
...
#define HM_FRM_CONFIG_PEER_ADD_NEW(frm_buf_tgt, frm_dst_addr) \
	HM_FRM_NEW((frm_buf_tgt), 16, ++hm_frm_cnt, 0x01, (frm_dst_addr)); \
...
#define HM_FRM_CONFIG_PEER_LIST_REQ_NEW(frm_buf_tgt, frm_dst_addr) \
	HM_FRM_NEW((frm_buf_tgt), 11, ++hm_frm_cnt, 0x01, (frm_dst_addr)); \
...
#define HM_FRM_CONFIG_PARAM_REQ_NEW(frm_buf_tgt, frm_dst_addr) \
	HM_FRM_NEW((frm_buf_tgt), 16, ++hm_frm_cnt, 01, (frm_dst_addr)); \
...
#define HM_FRM_CONFIG_WRITE_OFFSET_IS(frm) ((frm).type == 0x01 && (frm).config_unknown_01.subtype == 0x07)
#define HM_FRM_CONFIG_WRITE_OFFSET_NEW(frm_buf_tgt, frm_dst_addr, data_count) \
	HM_FRM_NEW((frm_buf_tgt), 11 + (data_count), ++hm_frm_cnt, 0x01, (frm_dst_addr)); \
	(frm_buf_tgt).config_unknown_01.subtype = 0x07;
typedef struct {
	uint8_t channel;
	uint8_t subtype;
	uint8_t data[(HM_FRM_PAYLOAD_SIZE - 2) / sizeof(uint8_t)];
} hm_frm_config_write_offset_t;
...
#define HM_FRM_CONFIG_WRITE_INDEX_NEW(frm_buf_tgt, frm_dst_addr, param_pairs_count) \
	HM_FRM_NEW((frm_buf_tgt), 11 + (param_pairs_count) * sizeof(hm_parameter_pair_t), ++hm_frm_cnt, 0x01, (frm_dst_addr)); \
...
#define HM_FRM_ACK_NACK_SUBTYPE_ACK_PARAM_IGNORED 0x02
...
#define HM_FRM_ACK_NACK_NEW(frm_buf_src, frm_buf_tgt, ack_nack_subtype) \
	HM_FRM_NEW((frm_buf_tgt), (ack_nack_subtype) == HM_FRM_ACK_NACK_SUBTYPE_ACK_STATUS ? 14 : 10, (frm_buf_src).cnt, 0x02, (frm_buf_src).addr_src); \
...
typedef struct {
	...
	uint8_t peer_channel;
} hm_frm_info_peer_list_onelink_t;
#define HM_FRM_INFO_PEER_LIST_IS(frm) ((frm).type == 0x10 && (frm).info_peer_list.subtype == 0x01)
#define HM_FRM_INFO_PEER_LIST_NEW(frm_buf_tgt, frm_cnt, frm_dst_addr) \
	HM_FRM_NEW((frm_buf_tgt), 14, (frm_cnt), 0x10, (frm_dst_addr)); \
	(frm_buf_tgt).info_peer_list.subtype = 0x01;
#define HM_FRM_INFO_PEER_LIST_GET_LEN(peers_count) (10 + (peers_count) * (sizeof(hm_frm_info_peer_list_onelink_t)))
typedef struct {
	uint8_t subtype;
	hm_frm_info_peer_list_onelink_t links[4];
...
#define HM_FRM_INFO_PARAM_RESPONSE_PAIRS_NEW(frm_buf_tgt, frm_cnt, frm_dst_addr, param_pairs_count) \
	HM_FRM_NEW((frm_buf_tgt), 10 + (param_pairs_count) * sizeof(hm_parameter_pair_t), (frm_cnt), 0x10, (frm_dst_addr)); \
...
#define HM_FRM_INFO_PARAM_RESPONSE_SEQ_NEW(frm_buf_tgt, frm_cnt, frm_dst_addr, param_count) \
	HM_FRM_NEW((frm_buf_tgt), 11 + (param_count) * sizeof(uint8_t), (frm_cnt), 0x10, (frm_dst_addr)); \
...
#define HM_FRM_RESET_NEW(frm_buf_tgt, frm_dst_addr) \
	HM_FRM_NEW((frm_buf_tgt), 11, ++hm_frm_cnt, 0x11, (frm_dst_addr)); \
...
#define HM_FRM_SWITCH_NEW(frm_buf_tgt, frm_dst_addr) \
	HM_FRM_NEW((frm_buf_tgt), 15, ++hm_frm_cnt, 0x3e, (frm_dst_addr));
typedef struct {
	uint24hm_t addr_dst;
	uint8_t unknown;
	uint8_t channel;
...
#define HM_FRM_REMOTE_NEW(frm_buf_tgt, frm_dst_addr) \
	HM_FRM_NEW((frm_buf_tgt), 11, ++hm_frm_cnt, 0x40, (frm_dst_addr));
typedef struct {
	uint8_t button:6;
...
typedef struct {
	...
	union {
		...
		hm_frm_config_write_offset_t config_write_offset;
